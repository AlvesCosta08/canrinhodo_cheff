...
<div class="printer-ticket">
    ...
    for ($i = 0; $i < count($dados); $i++) {
        ...
        // Buscar sigla da variação
        $query2 = $pdo->query("SELECT * FROM variacoes where id = '$variacao'");
        $res2 = $query2->fetchAll(PDO::FETCH_ASSOC);
        $sigla_variacao = (@count($res2) > 0) ? '(' . $res2[0]['sigla'] . ')' : '';

        // Buscar grade da variação
        ...
        $res2 = $query2->fetchAll(PDO::FETCH_ASSOC);
        $sigla_grade = (@$res2[0]['texto'] != "") ? @$res2[0]['texto'] : '';

        // Buscar nome do produto
        $query2 = $pdo->query("SELECT * FROM produtos where id = '$id_produto'");
        $res2 = $query2->fetchAll(PDO::FETCH_ASSOC);
        $nome_produto = (@count($res2) > 0) ? $res2[0]['nome'] : $nome_produto_tab;
        ...
        // 🔹 Apenas exibir borda se for Pizza ou Kalzone
        $nome_borda = '';
        if ($categoria == 'Pizza' || $categoria == 'Kalzone') {
            if ($borda != "") {
        ...
        <div class="itens">
            <div align="left">
                <?php echo $quantidade ?> - <?php echo $nome_produto ?> <?php echo $sigla_variacao ?> <?php echo $sigla_grade ?>
            ...
            <?php if ($sabores > 1): ?>
                <span style="margin-left: 15px"><small><b>Atenção: </b><?php echo $sabores; ?> Sabores</small></span><br>
            <?php endif; ?>

            <?php if ($nome_borda != ''): ?>
                <span style="margin-left: 15px"><small><b>Borda: </b><?php echo $nome_borda; ?></small></span><br>
            <?php endif; ?>
            ...
            $total_reg2 = @count($res2);
            if ($total_reg2 > 0):
                $texto_adicional = ($total_reg2 > 1) ? "($total_reg2) Tipos de Adicionais" : "($total_reg2) Tipo de Adicional";
                ?>
            ...
            <!-- Outras grades (extras, massas, etc.) -->
            <?php
                ...
                for ($i20 = 0; $i20 < $total_reg20; $i20++):
                    ...
                    if ($total_reg2 > 0):
                        ...
                        for ($i2 = 0; $i2 < $total_reg2; $i2++) {
                            ...
                            $quant_item = ($tipo_item_grade == 'Múltiplo') ? "($quantidade_item)" : '';
                            echo ' <i><small>' . $quant_item . $nome_item . '</small></i>';
                            if ($i2 < $total_reg2 - 1) echo ', ';
                        }
                        echo '<br></div>';
                    endif;
            ...
            <?php if ($obs_item != ""): ?>
                <div align="left" style="margin-left: 15px">
                    <small><b>Observação: </b><?php echo $obs_item; ?></small>
                </div>
            <?php endif; ?>
        </div>
    <?php } ?>
    ...
    <?php if ($entrega == "Delivery"): ?>
        <div class="row valores">
            <div class="col-6">Total dos Itens</div>
    ...
    <?php if ($entrega == "Delivery"): ?>
        <div class="valores" align="center">
            ...
            <?php echo $rua_cliente ?> <?php echo $numero_cliente ?> <?php echo $complemento_cliente ?><br>
